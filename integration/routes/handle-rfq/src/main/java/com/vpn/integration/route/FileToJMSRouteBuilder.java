...
public class FileToJMSRouteBuilder extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("file://" + this.incomingFileDirectory).id("file->queue:rfq").setHeader("rfqId", XPathBuilder.xpath("//rfq/id/text()", String.class)).convertBodyTo(String.class, Charset.forName("UTF-8").name()).to("log:test")
        .to("jms:rfq");
	}
